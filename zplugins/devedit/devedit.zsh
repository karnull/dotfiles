...
# invoke $EDITOR, word-splitting it so values like "code --wait" work
runEditor() {
    if [[ -z "$EDITOR" ]]; then
        echo -e "\e[31mError:\e[0m \$EDITOR is not set."
        return 1
    fi
    ${=EDITOR} "$@"
}
...
readme() {
    ...
    local -a readme_files=("$project_root"/README*(N))
...
makefile() {
    ...
    runEditor "$project_root/Makefile"
}
...
gignore() {
    ...
    runEditor "$project_root/.gitignore"
}
...
vi() {
    ...
    # word-split $EDITOR so values like "nvim -u NONE" work
    local -a editor_cmd=(${=EDITOR})

    # -O<n> (vertical splits) requires a vim-family editor
    if [[ "${editor_cmd[1]:t}" != (vim|nvim|view|gvim|mvim) ]]; then
        echo -e "\e[31mError:\e[0m vi requires a vim-family \$EDITOR (got: ${editor_cmd[1]:t})."
        return 1
    fi

    "${editor_cmd[@]}" -O"$split_count" "${selected_files[@]}"
}
